...
namespace rn
{
	...
	Viewport::Viewport(QWidget* parent) :
		...
		trackball_(new Trackball(/*Trackball::VerticalInverse*/)),
		model_creator(new CylindricalModelCreator())
	{
	...
	void Viewport::resizeGL(int width, int height) {
		...
		trackball_->setSceneSize(scene_size_);
	...
	void Viewport::paintGL() {
		...
		if (session_) {
			...
			//glLoadIdentity();
			//glBegin(GL_TRIANGLES);
			//glTexCoord2d(0, 0); glVertex3i(0, 0, -100);
			//glTexCoord2d(1, 0); glVertex3i(0, h / 2, -100);
			//glTexCoord2d(1, 1); glVertex3i(w, 0, -100);
			//glEnd();
			//glLineWidth(1);
			//glColor3d(1.0, 1.0, 1.0);
			//glPopMatrix();
	...
	void Viewport::mouseMoveEvent(QMouseEvent* ev) {
		model_creator->onMouseMove(ev->x(), ev->y());
		updateGL();

		if (trackball_->isClicked()) {
			if (ev->buttons() & Qt::RightButton) {
				trackball_->rotate(ev->x(), ev->y());
			}

			glLoadIdentity();
			glMultMatrixf(trackball_->getMat());
			updateGL();
		}
	}
	...
	void Viewport::mousePressEvent(QMouseEvent* ev) {
		if (ev->button() == Qt::LeftButton) {
			model_creator->onMouseMove(ev->x(), ev->y());
			model_creator->onMousePress(ev->button());
			updateGL();
		}
			...
			trackball_->setLastMatrix(model_view_matr);
			trackball_->click(ev->x(), ev->y());
			updateGL();
		}
	}
	...
	void Viewport::mouseReleaseEvent(QMouseEvent* ev) {
		if (ev->button() == Qt::LeftButton) {
			model_creator->onMouseMove(ev->x(), ev->y());
			model_creator->onMouseRelease(ev->button());
			updateGL();
		}
		else if (ev->button() == Qt::RightButton) {
			trackball_->release();
